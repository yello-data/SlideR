...
^[3]: Per veure totes les característiques del YAML, [veure](https://bookdown.org/yihui/rmarkdown/html-document.html)
...
Exemple: El nivell de democràcia de Denmark l'any 2020 era de:

```{r echo = T, eval = F}
`r polity$country[polity$country == "Denmark" & year == 2020]`
```

Exemple: El nivell de democràcia de r params$country l'any r params$year era de

```{r echo = T, eval = F}
polity$country[polity$country == params$country & year == params$year]
```

Exemple 1: El règim amb més durada és:

```{r eval = F, echo = T}
`p5$country[which(p5$durable == max(p5$durable, na.rm = T))]`
```
...
```{r eval = F, echo = T}
Com observem a la taula X, el nivell de democràcia de Dinamarca l'any `r params$year` és de `r p5$polity2`.
```
...
[d](https://getbootstrap.com/docs/4.0/components/alerts/)
```
...
Per referenciar[^3] la secció del document:
...
-   Entre claudàtors `[@Singer1987]`, ens posarà (Singer 1987).
-   Sense claudàtors `@Singer1987`, ens posarà Singer (1987).
...
-   `col.names = c("Noms", "Ben", "Posats)`
-   `align = c("ccc")`
-   `caption = "Títol"`

[^5]

[^5]: Taules de tota mena amb [KableExtra](https://cran.r-project.org/web/packages/kableExtra/vignettes/awesome_table_in_html.html)
